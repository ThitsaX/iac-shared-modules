...
resource "null_resource" "configure-nexus" {
  provisioner "local-exec" {
    command     = "ansible-galaxy collection install -f community.general:1.3.9 && ansible-playbook -i inventory nexus.yaml --extra-vars 'new_admin_pw=${local.nexus_admin_password} docker_group_listening_port=${var.docker_repo_listening_port}'"
